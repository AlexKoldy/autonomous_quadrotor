...
class OccupancyMap:
    ...
    def _metric_block_to_index_range(self, bounds, outer_bound=True):
        ...
        min_index_res = np.nextafter(
            self.resolution, sign * np.inf
        )  # Use for lower corner.
        max_index_res = np.nextafter(
            self.resolution, -sign * np.inf
        )  # Use for upper corner.
    ...
    def _init_map_from_world(self):
        ...
        for block in self.world.world.get("blocks", []):
            ...
            block_rect = Rectangle(
                [extent[1], extent[3], extent[5]], [extent[0], extent[2], extent[4]]
            )
            # Get index range that is definitely occupied by this block.
            (inner_min_index, inner_max_index) = self._metric_block_to_index_range(
            ...
            (outer_min_index, outer_max_index) = self._metric_block_to_index_range(
    ...
    def _draw_voxel_face(self, ax, index, direction):
        ...
        f = shapes.Face(
            ax, face, alpha=0.1, linewidth=1, edgecolors="k", facecolors="b"
        )
...
if __name__ == "__main__":
    ...
    world = World.random_forest(
        world_dims=(5, 5, 5), tree_width=0.1, tree_height=5, num_trees=10
    )
    ...
    oc = OccupancyMap(world, (0.2, 0.2, 0.5), 0.1)
